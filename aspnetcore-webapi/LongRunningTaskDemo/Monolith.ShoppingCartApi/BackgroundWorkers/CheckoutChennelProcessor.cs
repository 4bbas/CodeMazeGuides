...
public class CheckoutChennelProcessor : BackgroundService
{
    ...
    public CheckoutChennelProcessor(IStockValidator stockValidator,
    ...
    private async Task ProcessItemAsync(QueueItem item)
    {
        ...
        if (!await _stockValidator.ValidateAsync(item.Request.LineItems))
        {
            ...
            response.Message = $"{item.OrderId} item not available in stock";
